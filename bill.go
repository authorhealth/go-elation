...
func (b *BillService) Create(ctx context.Context, create *BillCreate) (*CreatedBill, *http.Response, error) {
...
func (b *BillService) Get(ctx context.Context, id int64) (*Bill, *http.Response, error) {
	ctx, span := b.client.tracer.Start(ctx, "get bill", trace.WithSpanKind(trace.SpanKindClient))
	defer span.End()

	bill := &Bill{}

	res, err := b.client.request(ctx, http.MethodGet, fmt.Sprintf("/bills/%d", id), nil, nil, &bill)
	if err != nil {
		span.RecordError(err)
		span.SetStatus(codes.Error, "error making request")
...
type FindBillOptions struct {
	*Pagination

	AssignedPhysician []int64   `url:"assigned_physician,omitempty"` //: [64811630594]
	BillID            []int64   `url:"bill_id,omitempty"`            //: [143945720594544]
	FromServiceDate   time.Time `url:"from_service_date,omitempty"`  //: "2016-05-23T17:50:50Z"
	ToServiceDate     time.Time `url:"to_service_date,omitempty"`    //: "2016-05-23T17:50:50Z"
	Patient           []int64   `url:"patient,omitempty"`            //: [141289139994622]
	SigningPhysician  int64     `url:"signing_physician,omitempty"`  //: 64811630594
	VisitNoteID       []int64   `url:"visit_note_id,omitempty"`      //: 12314823928
}

func (b *BillService) Find(ctx context.Context, opts *FindBillOptions) (*Response[[]*Bill], *http.Response, error) {
	ctx, span := b.client.tracer.Start(ctx, "find bills", trace.WithSpanKind(trace.SpanKindClient))
	defer span.End()

	out := &Response[[]*Bill]{}

	res, err := b.client.request(ctx, http.MethodGet, "/bills", opts, nil, &out)
	if err != nil {
		span.RecordError(err)
		span.SetStatus(codes.Error, "error making request")
