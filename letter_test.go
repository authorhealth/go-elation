...
func TestLetterService_Find(t *testing.T) {
	assert := assert.New(t)

	opts := &FindLettersOptions{
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		assert.Equal("/letters", r.URL.Path)
		...
		b, err := json.Marshal(Response[[]*Letter]{
			Results: []*Letter{
				{
					ID: 1,
				},
...
func TestLetterService_Get(t *testing.T) {
	assert := assert.New(t)
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		assert.Equal(http.MethodGet, r.Method)
		assert.Equal("/letters/"+strconv.FormatInt(id, 10), r.URL.Path)
